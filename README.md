...
Finally, we will demonstrate how to include external libraries and use a build tool to bundle and minify the code. We will cover server-side programming with Node.js and Express.js, and discuss relational databases along with basic security requirements and safeguards.
...
- This repository uses the "Live Server" extension by Ritwick Dey. After opening the repository in VS Code, you will be prompted to install the recommended extensions. If the prompt does not appear, click on "Extensions" (refer to the screenshot below) in the left panel and install it manually.

- Alternatively, you can run a web server via `python3 -m http.server 5500` (requires Python to be installed).

<img src="img/readme/vscode-left-panels.png" style="width:50%"/>
...
On the server side, we will use Express.js, the MariaDB relational database, and `bcrypt` for password hashing.

## First Run

If you open the local file `index.html` in your favorite browser, you will notice that the site does not function correctly: the button does nothing, and the drawing area is black. Open the browser's console to check for any errors. Depending on your browser, you might see messages such as "Failed to load resource: Origin null is not allowed by Access-Control-Allow-Origin." This occurs because the browser does not have permission to access other local files, that are referred from `index.html`.
...
_The screenshot illustrates the example site. Users can define a 2D vector, which is rendered live with each change. Additionally, the vector can be normalized using a dedicated button._
...
| `dist/`         | Bundled/compiled/minified JavaScript output                             |
| `img/`          | Images                                                                  |
...
1. Select `Run and Debug` on the left panel in VS Code.
2. Start the web server as described in the "First Run" section above.
3. Select `index.html` in the top-left panel of VS Code.
4. Enable debugging breakpoints: open a JavaScript file and click to the left of the line number (refer to the screenshot below).
5. Press [F5] or go to [Run] &rarr; [Start Debugging] in the main menu.
...
1. Select `Run and Debug` on the left panel in VS Code.
2. Select `node: current file` from the drop-down menu in the top-left corner of VS Code.
3. Enable debugging breakpoints by opening a JavaScript file and clicking to the left of the line number.
4. Press [F5] or go to [Run] → [Start Debugging] in the main menu.
...
Currently, our implementation in file `index.html` imports all JavaScript files as `module`. To use the minified version, comment out `<script type="module"...` and uncomment the lines to include the minified version.
...
The provided template code does not use any external dependencies except for `esbuild`. In larger projects, existing libraries should be used to avoid reinventing the wheel.
...
In this tutorial, we will use Express.js as the server. The example code will initially demonstrate how to store data in a local file. Later, we will use a relational database (MariaDB) to store user login data.
...
Install express.js from a terminal window:
...
Stop and restart the server. In the browser console, you'll notice that loading the module from `http://localhost:3000/src/index.js` is blocked. This happens because only the root (i.e., `index.html`) is being served. To resolve this, we need to add a route to serve static files.
...
The following code defines a POST route that receives a string from the client and saves it to a local file. Insert the following code before `app.listen(...)`.
...
We will now extend our page to include a user login feature. The main vector-editing page will only be visible if a user logs in successfully.

We use `MariaDB` as our database to store usernames and passwords.

The example code provides a login page that asks for a username and password. When the user clicks `login` and the provided credentials are correct, the main page will open.
...
Next, we need to install the database, and then add a route to `server.js`.

Please note that some commands will use the name `mysql` instead of `mariadb`. Install it on Linux as follows:
...
_Note that the attributes `loggedin` and `username` are self-chosen names._
...
Currently, the username input field is vulnerable to SQL injection. Example input string:

```
"; update Users set password="HACKED
```

To mitigate SQL injection risks, we should disable `multipleStatements` by setting it to `false` in the `createConnection` call. Since the example uses two statements (separated by `;`), the example attack is now prevented.
...
Create a new file named `bcrypt-test.js` with the following contents and run it with `node bcrypt-test.js` to generate the hash for the password `1337`. For simplicity, you can also temporarily add the following lines to `server.js`.
