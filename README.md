...
_Note: some of the text in this Readme was generated by an LLM. Everything has been revived by a human = me._

## Installation

Download and install [Visual Studio Code (VS Code)](https://code.visualstudio.com/download).

Clone this repository via `git clone https://github.com/andreas-schwenk/web-template.git`, or use [GitHub Desktop](https://desktop.github.com).

Open VS Code and open the example project via `File` &rarr; `Open` in the main menu.

We will need to run a local web server. There are several options to do that. The most easy way is to install and use an VS Code extension.

- This repository uses the "Live Server" extension by Ritwick Dey. After opening the repository in VS Code, you will be asked, if you like to install the recommended extensions. If the dialog does not appear, click on "Extensions" on the left panel then and install it.
...
The only dependency we use here is `esbuild`, an extremely fast JavaScript bundler and minifier. Shortly, it transpiles the sources from directory `src/` to a single, small JavaScript file in directory `dist/`. But currently, our project is set up as `module`, so we will not necessarily need a bundler. Refer to section "Minification" below, for a more detailed description.

# First Run and Debugging

If you open `index.html` in your favorite browser, you will notice that the site will not work: The button has no function, as well as the drawing area is black. Open the console of the browser to observe for any errors. Depending on your browser, for example "Failed to load resource: Origin null is not allowed by Access-Control-Allow-Origin." is shown. The browser does not have the access rights to other local files.

Start a local webserver at port `5500` (if you like to use another port number, make sure to change it in file `.vscode/launch.json`).

If you installed the `Live Server` extension in VS Code, then click no `Go Live` in the bottom-right in the VS Code window. The `Chrome` browser opens, and the page is working successfully.

Otherwise, start a web server e.g. with `python3 -m http.server 5500` and type `localhost:5500` in the URL of your browser.
